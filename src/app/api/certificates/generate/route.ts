...
const BLUE      = rgb(0.043, 0.290, 0.486)   // #0B4A7C
const BLUE_LITE = rgb(0.400, 0.710, 0.890)   // ~#66B5E3 border
const TEXT_DARK = rgb(0.102, 0.137, 0.251)   // #1A2340
const TEXT_MID  = rgb(0.239, 0.290, 0.420)   // #3D4A6B
const SIG_BLUE  = rgb(0.043, 0.290, 0.486)   // same as BLUE for sig text
...
function drawCenter(page: any, text: string, font: any, size: number, y: number, color: RGB, maxW?: number) {
  let t = text
  if (maxW) {
  ...
  page.drawText(t, { x: (PW - tw) / 2, y, size, font, color })
}
  ...
  try {
    ...
    const { courseTitle, fullName, completionDate, certNumber, language } =
      await req.json() as {
        ...
        certNumber: string; language: 'en' | 'es'; courseSlug?: string
      }
    ...
    page.drawRectangle({ x:14, y:14, width:PW-28, height:PH-28, borderColor:BLUE, borderWidth:2.5 })
    // 3. Inner border (light blue)
    page.drawRectangle({ x:20, y:20, width:PW-40, height:PH-40, borderColor:BLUE_LITE, borderWidth:1 })

    // 4. UN Emblem watermark — centered in the page
    const eSize = 220
    page.drawImage(embImg, {
      x: (PW - eSize) / 2, y: PH/2 - eSize/2 - 20,
      width: eSize, height: eSize, opacity: 0.08,
    })
    ...
    page.drawImage(logoImg, {
      x: PW - 30 - logoDims.width, y: PH - 30 - logoDims.height,
      width: logoDims.width, height: logoDims.height,
    })

    // ── Text content ─────────────────────────────────────────────────────────
    const bodyTop = PH - 30 - logoH - 50  // start below logo

    // "This Certificate is awarded to" / "Este Certificado es otorgado a"
    const line1 = isEs ? 'Este Certificado es otorgado a' : 'This Certificate is awarded to'
    drawCenter(page, line1, fLight, 14, bodyTop, TEXT_MID)

    // Full name — large bold
    let nameSize = 36
    while (fBold.widthOfTextAtSize(fullName, nameSize) > PW - 80 && nameSize > 20) nameSize -= 0.5
    drawCenter(page, fullName, fBold, nameSize, bodyTop - 54, TEXT_DARK, PW - 80)

    // Horizontal rule below name
    const ruleY = bodyTop - 68
    page.drawLine({ start: {x:30, y:ruleY}, end: {x:PW-30, y:ruleY}, thickness:1, color:BLUE })

    // "in recognition of the successful completion..." (2 lines)
    const midY = ruleY - 40
    const recLine1 = isEs
    ...
    // Course title — bold, wrapped
    const titleLines = wrap(courseTitle, fBold, 14, PW - 100)
    titleLines.forEach((line, i) => {
      const tw = fBold.widthOfTextAtSize(line, 14)
      page.drawText(line, { x: (PW - tw)/2, y: midY - 22 - i*20, size:14, font:fBold, color:TEXT_DARK })
    })
    ...
    page.drawImage(sigImg, {
      x: sigCX - sigDims.width/2, y: sigBaseY + 20,
      width: sigDims.width, height: sigDims.height,
    })

    page.drawLine({ start:{x:sigX, y:sigBaseY+16}, end:{x:sigX+sigBW, y:sigBaseY+16}, thickness:0.8, color:TEXT_DARK })

    const drawSig = (text: string, font: any, size: number, dy: number, color: RGB) => {
    ...
    const safeName = fullName.normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/\s+/g,'-').replace(/[^a-zA-Z0-9-_]/g,'')
